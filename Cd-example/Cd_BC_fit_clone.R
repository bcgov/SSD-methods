...
fit.all[[1]]
coef(fit.all[[1]]$dist.fits$lnorm)
vcov(fit.all[[1]]$dist.fits$lnorm)
#-------------------------------------------------------------------------------------------
...
BUGS.model <- readLines(file.path("..","DataCloning","fit.distr.clone.r"))
...
hc.log.se.clone <- dcsd(results)["hc.log"]
...
hc.se  <- fit.all[[1]]$pred.table[ fit.all[[1]]$pred.table$quantile==0.05, c("se")]
hc.log <- log(hc)
hc.log.ci <- log(hc.ci)
hc.log.se <- hc.se/hc
...
plot1 <- ggplot(data=plotdata, aes(y=ecdf))+
 ...
 #           color="blue",height=.05, size=.1)+
   ...
   geom_errorbarh(data=NULL, aes(xmin=hc.log.ci.clone[1], x=hc.log.clone, xmax=hc.log.ci.clone[2], y=.04), 
            color="blue",height=.05, size=1)
plot1
ggsave(plot1, file=file.path("Plots","BC_Cd_results.png"), h=6, w=6, unit="in",dpi=300)


c(mu, mu.se)
c(sigma, sigma.se)
c(hc.clone, hc.clone.se, hc.ci.clone)
temp<- vcov(results)
temp[c("mu","sigma"), c("mu","sigma")]

exp(mu-1.65*sigma)

# Why is the precision of the log(hc) so much better under data cloning?
hc.log.se
hc.log.se.clone

# conjecture that it is due to the negative covariance between the estimates
coef(results)[c("sigma","sigma_obs")]

temp <- vcov(results)
temp <- temp[c("mu","sigma"), c("mu","sigma")]
temp
sqrt(c(1, -1.645)%*% temp %*% c(1, -1.645))
