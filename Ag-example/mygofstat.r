# Correction in compute.cell.Chi2 when df=0.
# reported to package maintainers on 2016-09-18
...
mygofstat <- function (f, chisqbreaks, meancount, discrete, 
	fitnames=NULL) 
{
...
compute.gofstat.Chi2 <- function(sdata, n, distname, pdistname, estimate, fix.arg, chisqbreaks)
{
	...
	if (!is.null(chisqbreaks)) {
		...
		#browser() ***** ERROR HERE WHEN DF is 0 ******
		chisqpvalue <- ifelse(chisqdf>0, pchisq(chisq, df=chisqdf, lower.tail=FALSE), NA)
