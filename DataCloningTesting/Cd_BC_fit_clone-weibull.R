...
# endpoint using a WEIBULL distribution
...
fit.list <- list( 
#                  lnorm  = list(data=NULL, distr="lnorm",  method="mle"),
...
BUGS.model <- readLines(file.path("fit.distr.clone-weibull.r"))
...
# Good intial values for shape and Rscale are fount at
# http://stats.stackexchange.com/questions/230937/how-to-find-initial-values-for-weibull-mle-in-r
#       0.632 quantile of the distribution is λ
#
#   sx <- sort(x)
#   Fh <- ppoints(sx)
#   k0 <- lm(log(-log(1-Fh))~log(ws))$coefficients[2]
init.weibull <- function (x){
    # initialize values for fitting a Weibull distribution
    #browser()
    scale <- quantile(x, prob=.632)
    x <- sort(x)
    Fh <- ppoints(x)
    shape <- lm(log(-log(1-Fh))~log(x))$coefficients[2]
    c(shape, scale)
}
weibull.guess <- init.weibull(endpoints$Conc)

init.list <- list(
  list(sigma_obs=mean(species.sd$sd.log, na.rm=TRUE), mu_species=endpoint.mean$gmean,  # observation process
       shape=weibull.guess[1], Rscale=weibull.guess[2])  # initial values for log-normal distribution of species endpoints
...
monitor.list <- c("mu_species","sigma_obs","rank_mu_species", #"lik.data","lik.latent","lik",
                  "shape","Rscale",  # for weibull distribution
...
# What are the estiamted slope and set from the fit to the average
fit.all[[1]]$dist.fits$weibull
...
# We converted from the JAGs iscale paraeter to the Rscale parameter in the BUGS code

shape.clone    <- coef(results)["shape"]
shape.clone.se <- dcsd(results)["shape"]

scale.clone    <- coef(results)["Rscale"]
scale.clone.se <- dcsd(results)["Rscale"]

matrix(c(shape.clone, shape.clone.se, scale.clone, scale.clone.se), nrow=2, byrow=TRUE)
...
fitted.clone     <- data.frame(logConc=seq(min(c(log(endpoints$Conc),unlist(hc.log.ci.clone), unlist(hc.log.ci))),
                                           max(log(endpoints$Conc)),length.out=100))
fitted.clone$cdf <- pweibull(exp(fitted.clone$logConc), scale=scale.clone, shape=shape.clone)
head(fitted.clone)

# Compute the fitted curve when the geometric mean was used
names(coef(fit.all[[1]]$dist.fits$weibull))
fitted.avg     <- data.frame(logConc=fitted.clone$logConc)
fitted.avg$cdf <- pweibull(exp(fitted.avg$logConc), 
                           shape=coef(fit.all[[1]]$dist.fits$weibull)["shape"], 
                           scale=coef(fit.all[[1]]$dist.fits$weibull)["scale"])
head(fitted.avg)
...
plot1 <- ggplot(data=plotdata, aes(y=ecdf))+
   ggtitle("Fit of WEIBULL to BC Cd data (red=AVG, blue=MLE)")+
 ...
 #           color="blue",height=.05, size=.1)+
   ...
   geom_errorbarh(data=NULL, aes(xmin=hc.log.ci.clone[1], x=hc.log.clone, xmax=hc.log.ci.clone[2], y=.04), 
            color="blue",height=.05, size=1)
plot1
ggsave(plot1, file=file.path("Plots","BC_Cd_results-weibull.png"), h=6, w=6, unit="in",dpi=300)
